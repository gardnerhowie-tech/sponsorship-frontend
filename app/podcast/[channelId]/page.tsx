...
export default async function PodcastProfilePage({
  ...
  const classification =
    generateClassificationSummary({
      ...
      commentSignal:
        Number(
          data.comment_signal || 0
        ),

      responsivenessSignal:
        Number(
          data.responsiveness_signal || 0
        ),
      ...
      substantiveEngagement:
        Number(
          raw.substantive_engagement_component || 0
        ) / 10,

      behaviouralChange:
        Number(
          raw.behavioural_change_component || 0
        ) / 10,

      parasocial:
        Number(
          raw.parasocial_component || 0
        ) / 10,

      audienceInteraction:
        Number(
          raw.audience_interaction_component || 0
        ) / 10,
    });

  const trustScore = Math.round(
    Number(
      data.trust_index_score || 0
    ) * 100
  );
